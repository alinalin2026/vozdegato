...
import { MONTHLY_AMOUNTS, findTier } from "../shared/donaciones";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!stripe) {
    return res.status(503).json({
      error: "Los pagos todavía no están activados. Escríbenos a hola@vozdegato.com.",
    });
  }
  ...
  if (req.method === "GET") {
    ...
    if (!id) return res.status(400).json({ error: "Falta el identificador de la sesión." });
    try {
      ...
      return res.status(404).json({ error: "No hemos encontrado esa donación." });
    }
  }
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      return res.status(502).json({
        error: "No hemos podido abrir el pago. Inténtalo de nuevo en unos minutos.",
      });
    }
  }
